...
const prepareMessagesForAnalysis = (messages) => {
  ...
  const timeAnalysis = {
    ...
    endDate: conversations[conversations.length - 1]?.timestamp,
  ...
  conversations.forEach(msg => {
    ...
    timeAnalysis.messagesByHour[hour]++;
    timeAnalysis.messagesByDay[day] = (timeAnalysis.messagesByDay[day] || 0) + 1;
  });

  const conversationText = conversations
    .map(msg => `[${formatDate(msg.timestamp)}] ${msg.name_user}: ${msg.text}`)
    .join('\n');
